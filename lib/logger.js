...
Logger.prototype.to = function (device) {
  return this.set('@toDevice', device);
};
Logger.prototype.from = function (device) {
  return this.set('@fromDevice', device);
};
Logger.prototype.source = function (ip) {
  return this.set('@source', ip);
};
Logger.prototype.path = function (path) {
  return this.set('@interface', path);
};
Logger.prototype.param = function (param) {
  return this.set('@param', param);
};
Logger.prototype.detail = function (content) {
  return this.set('@detail', content);
};
Logger.prototype.error = function (message, type) {
  return this.set('@errMsg', message).set('@errType', type);
};
...
var middleware = function (req, res, next) {
  ...
  if (device === null) {
    errMsg = 'invalid sign';
    Logger().from('invalid').path(req.path).param(param).detail({}).error(errMsg, 'simple').send();
    return res.end({status: -1, msg: errMsg});
  }
  Logger().from(device).path(req.path).param(param).detail({}).send();
};
